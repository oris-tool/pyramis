...
/**
 * This class supports the analysis of the HSMP models with cycles 
...
public class TFLCyclesAnalysis {
	...
	public static void test1() {    
		...
		// LOOPS determines the number of iterations the cyle is executed for
			...
			for(int cycleType=3;cycleType<4;cycleType++) {
				for(int rejPeriodI=0;rejPeriodI<4;rejPeriodI++) {
					...
					try {
						...
						//HSMP
						//Build the model
						HierarchicalSMP model = TFLCycles.build(rejPeriod,cycleType,parallel,depth,seq,expolSame, lastB,LOOPS);
						...
						Set<String> zeros =TFLCycles.zeros;
						...
						TFLCyclesGroundTruth.sampleFromTime(rejPeriodI,LOOPS, cycleType, time, parallel,depth,seq,expolSame,lastB);
					}
					catch(Exception e){
