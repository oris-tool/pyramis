			...
			for(int i=HSMP_JournalCycles.depthS+1;i>=0;i--) {
				for(State s: compS) {
					...
					if(s.getDepth()==i-1) {
						...
						if(!sCycle) {
						...
						}else if(fL.get(fL.size()-1).equals(s)||(leaves>=2 && sL.get(fL.size()-1).equals(s))||(leaves==3 && tL.get(fL.size()-1).equals(s))){

							System.out.println(fL.get(fL.size()-1).getName());
							
							System.out.println(fL.get(fL.size()-1).getNextStates());
							
							LinkedList<State> l;
							State cr=s;
							int ind=0;
							if(fL.get(fL.size()-1).equals(s)) {
								l=fL;
							}
							else if(sL.get(fL.size()-1).equals(s)) {
								l=sL;
							}else {
								l=tL;
							}
							...
							System.out.println(((CompositeState)s).getRegions().get(0).getInitialState().getName());
