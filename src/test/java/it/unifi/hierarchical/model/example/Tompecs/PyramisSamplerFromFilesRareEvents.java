 ...
 *  
 */
public class PyramisSamplerFromFilesRareEvents {
	...
	public static void sample(long timeSS, int parallel,int depth,int seq,boolean expolSame, RegionType lastB){
		...
		HierarchicalSMP model = HSMP_JournalNewReview.build(parallel,depth,seq,true, lastB);
		...
		Set<String> sList = HSMP_JournalNewReview.getStates();
		...
		while(toEndTime>0 ) {
			...
			if(!changed && !firstCh) {
				...
				String print= g+"CHANGEDp-"+parallel+"_d-"+(depth+1)+"_s-"+seq+"_Final-"+lastB+"_"+(i*RUNS_AT_A_TIME)+"rare.txt";
	...
	public static int samplerOut(Map<String,Double> probabs, Sampler sampler, int RUNS_AT_A_TIME,State initial, RegionType lastB) {
		...
		while(countDown>0) {
			...
			Set<State> compS = HSMP_JournalNewReview.compS;

			Set<State> expS = HSMP_JournalNewReview.expS;
			Set<State> expDiffS = HSMP_JournalNewReview.expDiffS;
			Map<State,Region> regMap = HSMP_JournalNewReview.regMap;
			...
			for(State s: expS) {
				...
				State doub = HSMP_JournalNewReview.doublesMap.get(s);
				State win;
				double v;
				if(!expDiffBool) {
				...
				for(int q=1;q<HSMP_JournalNewReview.sequenceS;q++) {
					win= win.getNextStates().get(0);
					if(!expDiffBool) {
						v = sampler.getSample().doubleValue();
						samp.put(win, v);
						samp.put(HSMP_JournalNewReview.doublesMap.get(win), 0.);
					}else {
						v = sampler.getSample().doubleValue();
						samp.put(win, v);
						samp.put(HSMP_JournalNewReview.doublesMapFrom2.get(win), 0.);
			...
			for(int i=HSMP_JournalNewReview.depthS;i>=0;i--) {
				for(State s: compS) {
					if(s.getDepth()==i-1) {
						Double min=100000.;
						Double max=0.;
						for(int r=0; r< HSMP_JournalNewReview.parallelS;r++) {
	...
	private static void topDown(Region r, Double timeToEnd, Map<State,Double> samp,Map<String,Double> probabs ) {
		...
		if(init.getDepth()!=HSMP_JournalNewReview.depthS) {
